...
import { Settings, Share2, MoreVertical, Video, Copy, Menu, FileText, Users, Edit3 } from "lucide-react";
...
export default function ClassroomDetail() {
  ...
  useEffect(() => {
    const fetchClassroom = async () => {
      try {
        const token = Cookies.get("token");
        // Student gọi endpoint riêng để không lộ joinCode
        const endpoint = role === "teacher"
          ? `${API_BASE}/api/v1/classes/${classId}`
          : `${API_BASE}/api/v1/students/classes/${classId}`;
